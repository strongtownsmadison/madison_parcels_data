    ...
    map.on('mousemove', (event) => {
        ...
        document.getElementById('pd').innerHTML = parcels.length
            ? `<p>${parcels[0].properties.Address}</p>
                <p><strong>${(parcels[0].properties.taxes_per_sq_foot * 43560).toLocaleString("en-US", {
                style: "currency",
                currency: "USD"
            })}</strong> per acre</em></p>
                <p>${(parcels[0].properties.percentile / 100).toLocaleString("en-US", {
                style: "percent",
                minimumFractionDigits: 1,
                maximumFractionDigits: 2
            })} of parcels contribute fewer dollars per acre.</p>`
